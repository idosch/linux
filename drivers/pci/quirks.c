...
/*
 * On NVIDIA Spectrum-3 devices the device is reset following a bus reset, but
 * the firmware does not recover from it, making it impossible for the host to
 * communicate with it. Prevent bus reset for these devices.
 */
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_MELLANOX, 0xcf70, quirk_no_bus_reset);
 ...
 * (i.e., they advertise NoSoftRst-). This transition seems to have no effect
 * on the device: It continues to be operational and network ports remain up.
 * Advertising this support makes it seem as if a PM reset is viable for these
 * devices. Mark it as unavailable to skip it when testing reset methods.
 */
