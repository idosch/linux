...
{
	u64 common_groups_count = ARRAY_SIZE(mlxsw_sp_trap_group_items_arr);
	 ...
	 */
	groups_count = common_groups_count + trap->specific_groups_count;
	...
	memcpy(trap->group_items_arr + common_groups_count,
	       trap->specific_group_items_arr,
	       elem_size * trap->specific_groups_count);
...
{
	u64 common_traps_count = ARRAY_SIZE(mlxsw_sp_trap_items_arr);
	 ...
	 */
	traps_count = common_traps_count + trap->specific_traps_count;
	...
	memcpy(trap->trap_items_arr + common_traps_count,
	       trap->specific_trap_items_arr,
	       elem_size * trap->specific_traps_count);
...
{
	u64 groups_count = ARRAY_SIZE(mlxsw_sp1_trap_group_items_arr);
	struct mlxsw_sp_trap *trap = mlxsw_sp->trap;

	trap->specific_group_items_arr = mlxsw_sp1_trap_group_items_arr;
	trap->specific_groups_count = groups_count;
...
{
	u64 traps_count = ARRAY_SIZE(mlxsw_sp1_trap_items_arr);
	struct mlxsw_sp_trap *trap = mlxsw_sp->trap;

	trap->specific_trap_items_arr = mlxsw_sp1_trap_items_arr;
	trap->specific_traps_count = traps_count;
...
{
	u64 groups_count = ARRAY_SIZE(mlxsw_sp2_trap_group_items_arr);
	struct mlxsw_sp_trap *trap = mlxsw_sp->trap;

	trap->specific_group_items_arr = mlxsw_sp2_trap_group_items_arr;
	trap->specific_groups_count = groups_count;
...
{
	u64 traps_count = ARRAY_SIZE(mlxsw_sp2_trap_items_arr);
	struct mlxsw_sp_trap *trap = mlxsw_sp->trap;

	trap->specific_trap_items_arr = mlxsw_sp2_trap_items_arr;
	trap->specific_traps_count = traps_count;
