...
struct mlxsw_sp_span_trigger_ops {
	...
	bool (*match)(struct mlxsw_sp_span_trigger_entry *trigger_entry,
		      enum mlxsw_sp_span_trigger trigger,
					     ...
					     u16 policer_id)
{
	...
	if (refcount_read(&span->policer_id_base_ref_count)) {
		if (policer_id >= span->policer_id_base + span->entries_count)
			return -EINVAL;
				   ...
				   bool egress)
{
	 ...
	 * does the mirroring.
	 */
	err = mlxsw_sp_span_port_buffsize_update(mlxsw_sp_port,
						 mlxsw_sp_port->dev->mtu);
	if (err)
...
static bool
mlxsw_sp_span_trigger_port_match(struct mlxsw_sp_span_trigger_entry *
				 trigger_entry,
				 enum mlxsw_sp_span_trigger trigger,
				 struct mlxsw_sp_port *mlxsw_sp_port)
{
...
mlxsw_sp_span_trigger_port_ops = {
	...
	.match = mlxsw_sp_span_trigger_port_match,
...
static bool
mlxsw_sp1_span_trigger_global_match(struct mlxsw_sp_span_trigger_entry *
				    trigger_entry,
				    enum mlxsw_sp_span_trigger trigger,
				    struct mlxsw_sp_port *mlxsw_sp_port)
{
...
mlxsw_sp1_span_trigger_global_ops = {
	...
	.match = mlxsw_sp1_span_trigger_global_match,
...
static bool
mlxsw_sp2_span_trigger_global_match(struct mlxsw_sp_span_trigger_entry *
				    trigger_entry,
				    enum mlxsw_sp_span_trigger trigger,
				    struct mlxsw_sp_port *mlxsw_sp_port)
{
...
mlxsw_sp2_span_trigger_global_ops = {
	...
	.match = mlxsw_sp2_span_trigger_global_match,
				   ...
				   *parms)
{
	...
	memcpy(&trigger_entry->parms, parms, sizeof(*parms));
				 ...
				 struct mlxsw_sp_port *mlxsw_sp_port)
{
	...
	list_for_each_entry(trigger_entry, &span->trigger_entries_list, list) {
		if (trigger_entry->ops->match(trigger_entry, trigger,
					      mlxsw_sp_port))
			return trigger_entry;
	}
