...
struct mlxsw_sp_nexthop_group {
	...
	union {
		...
		u32 id; /* only relevant when type is OBJ */
	};
...
static u32 mlxsw_sp_nexthop_group_hash_obj(const void *data, u32 len, u32 seed)
{
	...
	case MLXSW_SP_NEXTHOP_GROUP_TYPE_OBJ:
		return jhash(&nh_grp->id, sizeof(nh_grp->id), seed);
	default:
		WARN_ON(1);
		return 0;
	}
					   ...
					   struct mlxsw_sp_nexthop_group *nh_grp)
{
	...
	nexthop_set_hw_flags(mlxsw_sp_net(mlxsw_sp), nh_grp->id,
...
mlxsw_sp_nexthop4_group_info_init(struct mlxsw_sp *mlxsw_sp,
				  struct mlxsw_sp_nexthop_group *nh_grp)
{
	unsigned int nhs = fib_info_num_path(nh_grp->fi);
	struct mlxsw_sp_nexthop_group_info *nhgi;
	struct mlxsw_sp_nexthop *nh;
	int err, i;
	...
	nh_grp->nhgi = nhgi;
	nhgi->nh_grp = nh_grp;
	nhgi->gateway = mlxsw_sp_fi_is_gateway(mlxsw_sp, nh_grp->fi);
	nhgi->count = nhs;
	for (i = 0; i < nhgi->count; i++) {
		struct fib_nh *fib_nh;

		nh = &nhgi->nexthops[i];
		fib_nh = fib_info_nh(nh_grp->fi, i);
