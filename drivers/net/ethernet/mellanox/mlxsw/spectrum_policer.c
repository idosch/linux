...
enum mlxsw_sp_policer_family_type {
	MLXSW_SP_POLICER_FAMILY_TYPE_SINGLE_RATE,

	__MLXSW_SP_POLICER_FAMILY_TYPE_MAX,
	MLXSW_SP_POLICER_FAMILY_TYPE_MAX = __MLXSW_SP_POLICER_FAMILY_TYPE_MAX - 1,
};

struct mlxsw_sp_policer_family {
	enum mlxsw_sp_policer_family_type type;
...
struct mlxsw_sp_policer_family_ops {
	bool (*compare_params)(const struct mlxsw_sp_policer_family *family,
			       enum mlxsw_sp_policer_type type,
			       const struct mlxsw_sp_policer_params *params);
	bool (*compare_index)(const struct mlxsw_sp_policer_family *family,
			      enum mlxsw_sp_policer_type type,
			      u16 policer_index);
	...
	void (*policer_index_free)(struct mlxsw_sp_policer_family *family,
				   u16 policer_index);
...
struct mlxsw_sp_policer_core {
	struct mlxsw_sp_policer_family *family_arr[MLXSW_SP_POLICER_FAMILY_TYPE_MAX + 1];
...
static bool
mlxsw_sp_policer_single_rate_compare_params(const struct mlxsw_sp_policer_family *family,
					    enum mlxsw_sp_policer_type type,
					    const struct mlxsw_sp_policer_params *params)
{
	return type == MLXSW_SP_POLICER_TYPE_POLICY_ENGINE && params->bytes;
}

static bool
mlxsw_sp_policer_single_rate_compare_index(const struct mlxsw_sp_policer_family *family,
					   enum mlxsw_sp_policer_type type, u16 policer_index)
{
	return type == MLXSW_SP_POLICER_TYPE_POLICY_ENGINE &&
	       policer_index >= family->start_index &&
					  ...
					  struct netlink_ext_ack *extack)
{
	...
	u8 bs = mlxsw_sp_policer_burst_bytes_hw_units(params->burst);
...
static const struct mlxsw_sp_policer_family_ops mlxsw_sp_policer_single_rate_ops = {
	.compare_params		= mlxsw_sp_policer_single_rate_compare_params,
	.compare_index		= mlxsw_sp_policer_single_rate_compare_index,
...
static const struct mlxsw_sp_policer_family mlxsw_sp_policer_single_rate_family = {
	.type		= MLXSW_SP_POLICER_FAMILY_TYPE_SINGLE_RATE,
...
static const struct mlxsw_sp_policer_family *mlxsw_sp_policer_family_arr[] = {
	[MLXSW_SP_POLICER_FAMILY_TYPE_SINGLE_RATE]	= &mlxsw_sp_policer_single_rate_family,
};

static int
mlxsw_sp_policer_params_check(const struct mlxsw_sp_policer_family *family,
			      const struct mlxsw_sp_policer_params *params,
			      struct netlink_ext_ack *extack)
{
	...
	return family->ops->policer_params_check(family, params, extack);
}

static struct mlxsw_sp_policer_family *
mlxsw_sp_policer_family_find_by_params(struct mlxsw_sp *mlxsw_sp,
				       enum mlxsw_sp_policer_type type,
				       const struct mlxsw_sp_policer_params *params)
{
	int i;

	for (i = 0; i < MLXSW_SP_POLICER_FAMILY_TYPE_MAX + 1; i++) {
		struct mlxsw_sp_policer_family *family;

		family = mlxsw_sp->policer_core->family_arr[i];
		if (family->ops->compare_params(family, type, params))
			return family;
	}
...
static struct mlxsw_sp_policer_family *
mlxsw_sp_policer_family_find_by_index(struct mlxsw_sp *mlxsw_sp,
				      enum mlxsw_sp_policer_type type,
				      u16 policer_index)
{
	int i;
	...
	for (i = 0; i < MLXSW_SP_POLICER_FAMILY_TYPE_MAX + 1; i++) {
		...
		family = mlxsw_sp->policer_core->family_arr[i];
		if (family->ops->compare_index(family, type, policer_index))
			return family;
	}
			 ...
			 struct netlink_ext_ack *extack, u16 *p_policer_index)
{
	...
	family = mlxsw_sp_policer_family_find_by_params(mlxsw_sp, type, params);
	if (WARN_ON(!family))
		return -EINVAL;

	err = mlxsw_sp_policer_params_check(family, params, extack);
	if (err)
		return err;
			  ...
			  enum mlxsw_sp_policer_type type, u16 policer_index)
{
	...
	family = mlxsw_sp_policer_family_find_by_index(mlxsw_sp, type,
						       policer_index);
	if (WARN_ON(!family))
		return;
	...
	policer = idr_find(&family->policer_idr, policer_index);
	if (WARN_ON(!policer))
		goto out;
...
mlxsw_sp_policer_family_unregister(struct mlxsw_sp *mlxsw_sp,
				   struct mlxsw_sp_policer_family *family)
{
	mlxsw_sp->policer_core->family_arr[family->type] = NULL;
	family->ops->fini(family);
	mutex_destroy(&family->lock);
...
void mlxsw_sp_policers_fini(struct mlxsw_sp *mlxsw_sp)
{
	...
	for (i = MLXSW_SP_POLICER_FAMILY_TYPE_MAX; i >= 0; i--) {
		struct mlxsw_sp_policer_family *family;
