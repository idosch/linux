				  ...
				  struct netlink_ext_ack *extack)
{
	...
	*p_policer_index = policer_ref->policer->policer_index;
	...
	params.action_code = MLXSW_AFA_POLCNT_CODE;
	params.action_size = MLXSW_AFA_POLCNT_SIZE;
	params.is_police = true;
	act = mlxsw_afa_block_append_action_ext(block, &params);
	if (IS_ERR(act)) {
