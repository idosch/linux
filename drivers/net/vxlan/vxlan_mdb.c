...
struct vxlan_mdb_sg_remote_key {
	union vxlan_addr remote_ip;
	struct vxlan_mdb_entry_key group;
};

#define VXLAN_MDB_REMOTE_F_STAR_EXCL	BIT(0)
#define VXLAN_MDB_REMOTE_F_BLOCKED	BIT(1)
...
struct vxlan_mdb_remote {
	...
	u8 filter_mode;
	u8 rt_protocol;
	struct vxlan_mdb_sg_remote_key key;
	struct rhash_head rhnode;
...
static const struct rhashtable_params vxlan_mdb_sg_remote_rht_params = {
	.head_offset = offsetof(struct vxlan_mdb_remote, rhnode),
	.key_offset = offsetof(struct vxlan_mdb_remote, key),
	.key_len = sizeof(struct vxlan_mdb_sg_remote_key),
	.automatic_shrinking = true,
};
				    ...
				    struct br_mdb_entry *e)
{
	...
	if (remote->flags & VXLAN_MDB_REMOTE_F_STAR_EXCL)
...
static struct vxlan_mdb_remote *
vxlan_mdb_sg_remote_lookup(struct vxlan_dev *vxlan,
			   const struct vxlan_mdb_sg_remote_key *sg_key)
{
	return rhashtable_lookup_fast(&vxlan->mdb_sg_remote_tbl, sg_key,
				      vxlan_mdb_sg_remote_rht_params);
}
				 ...
				 struct vxlan_mdb_remote *remote)
{
	...
	remote->key.remote_ip = cfg->remote_ip;
	remote->key.group = cfg->group;
	INIT_HLIST_HEAD(&remote->src_list);

	if (vxlan_mdb_is_sg(&cfg->group)) {
		err = rhashtable_lookup_insert_fast(&vxlan->mdb_sg_remote_tbl,
						    &remote->rhnode,
						    vxlan_mdb_sg_remote_rht_params);
		if (err)
			goto err_rdst_fini;
...
static void vxlan_mdb_remote_fini(struct vxlan_dev *vxlan,
				  struct vxlan_mdb_remote *remote)
{
	if (vxlan_mdb_is_sg(&remote->key.group))
		rhashtable_remove_fast(&vxlan->mdb_sg_remote_tbl,
				       &remote->rhnode,
				       vxlan_mdb_sg_remote_rht_params);
			     ...
			     const union vxlan_addr *addr)
{
	...
	sg_cfg.vxlan = vxlan;
	sg_cfg.group.src = *addr;
	sg_cfg.group.dst = remote->key.group.dst;
	sg_cfg.group.vni = remote->key.group.vni;
	INIT_LIST_HEAD(&sg_cfg.src_list);
...
static void vxlan_mdb_remote_src_del(struct vxlan_dev *vxlan,
				     ...
				     struct vxlan_mdb_src_entry *ent)
{
	vxlan_mdb_remote_src_fwd_del(vxlan, remote, &ent->addr);
	vxlan_mdb_remote_src_entry_del(ent);
}
...
err_src_del:
	hlist_for_each_entry_safe(ent, tmp, &remote->src_list, node)
		vxlan_mdb_remote_src_del(cfg->vxlan, remote, ent);
	return err;
}
				      ...
				      struct vxlan_mdb_remote *remote)
{
	...
	hlist_for_each_entry_safe(ent, tmp, &remote->src_list, node)
		vxlan_mdb_remote_src_del(vxlan, remote, ent);
}
...
static void
vxlan_mdb_star_ex_config_init(struct vxlan_mdb_config *cfg,
			      struct vxlan_dev *vxlan,
			      const struct vxlan_mdb_entry_key *group,
			      const struct vxlan_mdb_remote *remote,
			      u16 nlflags)
{
	...
	cfg->group = *group;
	INIT_LIST_HEAD(&cfg->src_list);
	cfg->remote_ip = rd->remote_ip;
	cfg->remote_ifindex = rd->remote_ifindex;
	cfg->remote_vni = rd->remote_vni;
	cfg->remote_port = rd->remote_port;
	cfg->nlflags = nlflags;
	cfg->flags = VXLAN_MDB_REMOTE_F_STAR_EXCL;
	cfg->filter_mode = MCAST_INCLUDE;
	cfg->rt_protocol = RTPROT_KERNEL;
}

static void
vxlan_mdb_star_ex_sg_add(struct vxlan_dev *vxlan,
			 const struct vxlan_mdb_remote *remote,
			 const struct vxlan_mdb_sg_remote_key *sg_key)
{
	struct vxlan_mdb_config cfg;

	if (vxlan_mdb_sg_remote_lookup(vxlan, sg_key))
		return;

	vxlan_mdb_star_ex_config_init(&cfg, vxlan, &sg_key->group, remote,
				      NLM_F_CREATE);
	__vxlan_mdb_add(&cfg, NULL);
}

static void
vxlan_mdb_star_ex_sg_del(struct vxlan_dev *vxlan,
			 const struct vxlan_mdb_remote *remote,
			 const struct vxlan_mdb_sg_remote_key *sg_key)
{
	struct vxlan_mdb_remote *sg_remote;
	struct vxlan_mdb_config cfg;

	sg_remote = vxlan_mdb_sg_remote_lookup(vxlan, sg_key);
	if (!sg_remote || !(sg_remote->flags & VXLAN_MDB_REMOTE_F_STAR_EXCL) ||
	    WARN_ON_ONCE(sg_remote->rt_protocol != RTPROT_KERNEL))
		return;

	vxlan_mdb_star_ex_config_init(&cfg, vxlan, &sg_key->group, remote, 0);
	__vxlan_mdb_del(&cfg, NULL);
}

static void vxlan_mdb_star_g_mode_handle(struct vxlan_dev *vxlan,
					 const struct vxlan_mdb_remote *remote,
					 u8 filter_mode)
{
	struct vxlan_mdb_sg_remote_key sg_key = remote->key;
	struct vxlan_mdb_remote *remote_iter;
	struct vxlan_mdb_entry *mdb_entry;

	/* Depending on the filter mode, the remote either needs to be added to
	 * existing (S, G) entries or removed from them. Find all the relevant
	 * (S, G) entries by first finding the (*, G) MDB entry. Then, iterate
	 * over all of its remotes (skipping the current one) and for each such
	 * remote iterate over its source list.
	 */

	if (WARN_ON_ONCE(!vxlan_mdb_is_star_g(&remote->key.group)))
		return;

	mdb_entry = vxlan_mdb_entry_lookup(vxlan, &remote->key.group);
	if (!mdb_entry)
		return;

	list_for_each_entry(remote_iter, &mdb_entry->remotes, list) {
		struct vxlan_mdb_src_entry *ent;

		if (remote_iter == remote)
			continue;

		hlist_for_each_entry(ent, &remote_iter->src_list, node) {
			sg_key.group.src = ent->addr;
			switch (filter_mode) {
			case MCAST_EXCLUDE:
				vxlan_mdb_star_ex_sg_add(vxlan, remote,
							 &sg_key);
				break;
			case MCAST_INCLUDE:
				vxlan_mdb_star_ex_sg_del(vxlan, remote,
							 &sg_key);
				break;
			}
		}
...
static void
vxlan_mdb_sg_exclude_remotes_add(struct vxlan_dev *vxlan,
				 const struct vxlan_mdb_remote *remote)
{
	struct vxlan_mdb_entry_key group = remote->key.group;
	struct vxlan_mdb_sg_remote_key sg_key = remote->key;
	struct vxlan_mdb_remote *remote_iter;
	struct vxlan_mdb_entry *mdb_entry;

	/* A new (S, G, remote) entry was added and we need to find out which
	 * other remotes need to be added to the (S, G) entry for proper
	 * replication. Find these remotes by first finding the (*, G) MDB
	 * entry. Iterate over its remotes while skipping over the current one
	 * and those that are not in EXCLUDE filter mode. Then, for each remote
	 * check if there is already a (S, G, remote) entry and if not, add
	 * one.
	 */

	if (WARN_ON_ONCE(!vxlan_mdb_is_sg(&remote->key.group)))
		return;
	...
	list_for_each_entry(remote_iter, &mdb_entry->remotes, list) {
		struct vxlan_rdst *rd_iter = rtnl_dereference(remote_iter->rd);
		struct vxlan_rdst *rd = rtnl_dereference(remote->rd);
		struct vxlan_mdb_config cfg;

		if (remote_iter->filter_mode != MCAST_EXCLUDE ||
		    vxlan_addr_equal(&rd->remote_ip, &rd_iter->remote_ip))
			continue;

		sg_key.remote_ip = rd_iter->remote_ip;
		if (vxlan_mdb_sg_remote_lookup(vxlan, &sg_key))
			return;

		vxlan_mdb_star_ex_config_init(&cfg, vxlan, &sg_key.group,
					      remote_iter, NLM_F_CREATE);
		__vxlan_mdb_add(&cfg, NULL);
	}
}

static void
vxlan_mdb_sg_exclude_remotes_del(struct vxlan_dev *vxlan,
				 const struct vxlan_mdb_remote *remote)
{
	struct vxlan_mdb_entry_key group = remote->key.group;
	struct vxlan_mdb_remote *remote_iter, *tmp;
	struct vxlan_mdb_entry *mdb_entry;

	/* A (S, G, remote) entry is being deleted and we need to remove all
	 * the remotes that were added as STAR_EXCL to the (S, G) entry because
	 * of it. Find the (S, G) entry, iterate over all of its remotes and
	 * delete those marked as STAR_EXCL.
	 */

	mdb_entry = vxlan_mdb_entry_lookup(vxlan, &group);
	if (!mdb_entry || WARN_ON_ONCE(!vxlan_mdb_is_sg(&mdb_entry->key)))
		return;

	list_for_each_entry_safe(remote_iter, tmp, &mdb_entry->remotes, list) {
		struct vxlan_mdb_config cfg;

		if (!(remote_iter->flags & VXLAN_MDB_REMOTE_F_STAR_EXCL))
			continue;

		vxlan_mdb_star_ex_config_init(&cfg, vxlan, &group, remote_iter,
					      0);
		__vxlan_mdb_del(&cfg, NULL);
	}
}

static void vxlan_mdb_star_ex_add(struct vxlan_dev *vxlan,
				  const struct vxlan_mdb_remote *remote)
{
	/* STAR_EXCL MDB remote entries do not trigger the creation of more
	 * entries.
	 */
	if (remote->flags & VXLAN_MDB_REMOTE_F_STAR_EXCL)
		return;

	if (vxlan_mdb_is_star_g(&remote->key.group) &&
	    remote->filter_mode == MCAST_EXCLUDE) {
		/* The added MDB remote entry is (*, G) EXCLUDE, so the remote
		 * needs to be added to all the matching (S, G) entries for
		 * proper replication. If a specific source is blocked, then it
		 * already has a (S, G) entry and the entry will not be
		 * modified.
		 */
		vxlan_mdb_star_g_mode_handle(vxlan, remote, MCAST_EXCLUDE);
	} else if (vxlan_mdb_is_sg(&remote->key.group)) {
		/* All the (*, G) EXCLUDE remotes need to be added to the (S,
		 * G) MDB entry for proper replication, if not already there.
		 */
		vxlan_mdb_sg_exclude_remotes_add(vxlan, remote);
	}
}

static void vxlan_mdb_star_ex_del(struct vxlan_dev *vxlan,
				  const struct vxlan_mdb_remote *remote)
{
	/* STAR_EXCL MDB remote entries do not trigger the deletion of more
	 * entries.
	 */
	if (remote->flags & VXLAN_MDB_REMOTE_F_STAR_EXCL)
		return;
	...
	if (vxlan_mdb_is_star_g(&remote->key.group) &&
	    remote->filter_mode == MCAST_EXCLUDE) {
		/* The deleted MDB remote entry is (*, G) EXCLUDE, so the
		 * remote needs to be removed from all the (S, G) entries to
		 * which it was added as STAR_EXCL.
		 */
		vxlan_mdb_star_g_mode_handle(vxlan, remote, MCAST_INCLUDE);
	} else if (vxlan_mdb_is_sg(&remote->key.group)) {
		/* Remove all the (*, G) EXCLUDE remotes that were added to the
		 * (S, G) MDB entry as STAR_EXCL.
		 */
		vxlan_mdb_sg_exclude_remotes_del(vxlan, remote);
	}
}
				    ...
				    struct netlink_ext_ack *extack)
{
	...
	if (vxlan_mdb_is_star_g(&remote->key.group))
		vxlan_mdb_star_g_mode_handle(vxlan, remote, cfg->filter_mode);
				...
				struct netlink_ext_ack *extack)
{
	...
	vxlan_mdb_star_ex_add(cfg->vxlan, remote);
						...
						__be32 src_vni)
{
	 ...
	 * to flood such traffic together with broadcast and unknown unicast
	 * traffic.
	 */
	switch (ntohs(skb->protocol)) {
	case ETH_P_IP:
...
int vxlan_mdb_init(struct vxlan_dev *vxlan)
{
	...
	err = rhashtable_init(&vxlan->mdb_sg_remote_tbl,
			      &vxlan_mdb_sg_remote_rht_params);
	if (err)
		goto err_rhash_free;
