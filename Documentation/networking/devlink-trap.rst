...
.. list-table:: List of Generic Packet Traps
   ...
   * - ``uc_empty_tx_list_drop``
     ...
     - Traps packets with a unicast MAC that the device decided to drop in case
       after layer 2 forwarding there are no ports to transmit the packet
       through
   * - ``mc_empty_tx_list_drop``
     - ``drop``
     - Traps packets with a multicast MAC that the device decided to drop in
       case after layer 2 forwarding there are no ports to transmit the packet
       through
   * - ``uc_loopback_filter_drop``
     - ``drop``
     - Traps packets with a unicast MAC that the device decided to drop in case
       after layer 2 forwarding the packet should be transmitted through the
       same port from which it was received
