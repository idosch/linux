...
common_speeds_get()
{
	...
	if [[ $adver -eq 1 ]]; then
		local -a dev1_speeds=($(advertised_speeds_get $dev1 $with_mode))
		local -a dev2_speeds=($(advertised_speeds_get $dev2 $with_mode))
	else
		local -a dev1_speeds=($(supported_speeds_get $dev1 $with_mode))
		local -a dev2_speeds=($(supported_speeds_get $dev2 $with_mode))
	fi
