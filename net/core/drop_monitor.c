...
struct net_dm_alert_ops {
	...
	void (*trap_probe)(void *ignore, struct sk_buff *skb,
...
static void
net_dm_hw_trap_summary_probe(void *ignore, struct sk_buff *skb,
			     const char *trap_name, const char *trap_group_name,
			     struct net_device *input_dev,
			     const struct flow_action_cookie *fa_cookie)
{
	...
	for (i = 0; i < hw_entries->num_entries; i++) {
		hw_entry = &hw_entries->entries[i];
		if (!strncmp(hw_entry->trap_name, trap_name,
			     NET_DM_MAX_HW_TRAP_NAME_LEN - 1)) {
			hw_entry->count++;
			goto out;
		}
	...
	hw_entry = &hw_entries->entries[hw_entries->num_entries];
	strlcpy(hw_entry->trap_name, trap_name,
		NET_DM_MAX_HW_TRAP_NAME_LEN - 1);
	hw_entry->count = 1;
	hw_entries->num_entries++;
...
static const struct net_dm_alert_ops net_dm_alert_summary_ops = {
	...
	.trap_probe		= net_dm_hw_trap_summary_probe,
};
...
static struct net_dm_hw_metadata *
net_dm_hw_metadata_copy(const char *trap_name, const char *trap_group_name,
			struct net_device *input_dev,
			const struct flow_action_cookie *fa_cookie)
{
	const struct flow_action_cookie *n_fa_cookie;
	struct net_dm_hw_metadata *n_hw_metadata;
	const char *n_trap_group_name;
	const char *n_trap_name;

	n_hw_metadata = kzalloc(sizeof(*n_hw_metadata), GFP_ATOMIC);
	if (!n_hw_metadata)
		return NULL;

	n_trap_group_name = kstrdup(trap_group_name, GFP_ATOMIC);
	if (!n_trap_group_name)
		goto free_hw_metadata;
	n_hw_metadata->trap_group_name = n_trap_group_name;

	n_trap_name = kstrdup(trap_name, GFP_ATOMIC);
	if (!n_trap_name)
		goto free_trap_group;
	n_hw_metadata->trap_name = n_trap_name;

	if (fa_cookie) {
		size_t cookie_size = sizeof(*fa_cookie) + fa_cookie->cookie_len;

		n_fa_cookie = kmemdup(fa_cookie, cookie_size, GFP_ATOMIC);
		if (!n_fa_cookie)
			goto free_trap_name;
		n_hw_metadata->fa_cookie = n_fa_cookie;
	}

	n_hw_metadata->input_dev = input_dev;
	if (n_hw_metadata->input_dev)
		dev_hold(n_hw_metadata->input_dev);
...
static void
net_dm_hw_trap_packet_probe(void *ignore, struct sk_buff *skb,
			    const char *trap_name, const char *trap_group_name,
			    struct net_device *input_dev,
			    const struct flow_action_cookie *fa_cookie)
{
	...
	n_hw_metadata = net_dm_hw_metadata_copy(trap_name, trap_group_name,
						input_dev, fa_cookie);
	if (!n_hw_metadata)
		goto free;
...
static const struct net_dm_alert_ops net_dm_alert_packet_ops = {
	...
	.trap_probe		= net_dm_hw_trap_packet_probe,
};
...
{
	return register_trace_devlink_trap_report(ops->trap_probe, NULL);
}

static void net_dm_hw_probe_unregister(const struct net_dm_alert_ops *ops)
{
	unregister_trace_devlink_trap_report(ops->trap_probe, NULL);
