...
static int
devlink_trap_state_get_from_info(struct genl_info *info,
				 enum devlink_trap_state *p_trap_state)
{
	u8 val;

	if (!info->attrs[DEVLINK_ATTR_TRAP_STATE])
		return -EINVAL;

	val = nla_get_u8(info->attrs[DEVLINK_ATTR_TRAP_STATE]);
	switch (val) {
	case DEVLINK_TRAP_STATE_DISABLE: /* fall-through */
	case DEVLINK_TRAP_STATE_ENABLE: /* fall-through */
				...
				int flags)
{
	...
	if (nla_put_u8(msg, DEVLINK_ATTR_TRAP_STATE, trap_item->state))
		goto nla_put_failure;
...
static int devlink_trap_state_set(struct devlink *devlink,
				  struct devlink_trap_item *trap_item,
				  struct genl_info *info)
{
	...
	err = devlink_trap_state_get_from_info(info, &trap_state);
	if (err) {
		NL_SET_ERR_MSG_MOD(info->extack, "Invalid trap state");
		return -EINVAL;
	}

	enable = trap_state != DEVLINK_TRAP_STATE_DISABLE;
	err = trap_item->trap->state_set(devlink, trap_id, enable);
	if (err)
		return err;

	trap_item->state = trap_state;
	return 0;
}
					...
					struct genl_info *info)
{
	...
	err = devlink_trap_state_set(devlink, trap_item, info);
	if (err)
		return err;
			   ...
			   int flags)
{
	...
	if (nla_put_u8(msg, DEVLINK_ATTR_TRAP_GROUP_STATE, group_item->state))
		goto nla_put_failure;
...
static int
devlink_trap_group_state_get_from_info(struct genl_info *info,
				       enum devlink_trap_state *p_trap_state)
{
	u8 val;

	if (!info->attrs[DEVLINK_ATTR_TRAP_GROUP_STATE])
		return -EINVAL;

	val = nla_get_u8(info->attrs[DEVLINK_ATTR_TRAP_GROUP_STATE]);
	switch (val) {
	case DEVLINK_TRAP_STATE_DISABLE: /* fall-through */
	case DEVLINK_TRAP_STATE_ENABLE: /* fall-through */
	case DEVLINK_TRAP_STATE_REPORT:
...
static int
__devlink_trap_group_state_set(struct devlink *devlink,
			       struct devlink_trap_group_item *group_item,
			       enum devlink_trap_state trap_state)
{
	bool enable = trap_state != DEVLINK_TRAP_STATE_DISABLE;
	struct devlink_trap_item *trap_item;
	int err;

	list_for_each_entry(trap_item, &devlink->trap_list, list) {
		if (trap_item->trap->group_id != group_item->group->id)
			continue;
		err = trap_item->trap->state_set(devlink, trap_item->trap->id,
						 enable);
		if (err)
			return err;
		trap_item->state = trap_state;
...
static int
devlink_trap_group_state_set(struct devlink *devlink,
			     struct devlink_trap_group_item *group_item,
			     struct genl_info *info)
{
	enum devlink_trap_state trap_state;
	...
	err = devlink_trap_group_state_get_from_info(info, &trap_state);
	if (err) {
		NL_SET_ERR_MSG_MOD(info->extack, "Invalid trap group state");
		return -EINVAL;
	}

	err = __devlink_trap_group_state_set(devlink, group_item, trap_state);
	if (err)
		return err;

	group_item->state = trap_state;
					      ...
					      struct genl_info *info)
{
	...
	err = devlink_trap_group_state_set(devlink, group_item, info);
	if (err)
		return err;
...
static const struct nla_policy devlink_nl_policy[DEVLINK_ATTR_MAX + 1] = {
	...
	[DEVLINK_ATTR_TRAP_STATE] = { .type = NLA_U8 },
	[DEVLINK_ATTR_TRAP_GROUP_ID] = { .type = NLA_U16 },
	[DEVLINK_ATTR_TRAP_GROUP_STATE] = { .type = NLA_U8 },
};
				 ...
				 const struct devlink_trap *trap)
{
	...
	trap_item->state = trap->init_state;
	trap_item->trap = trap;
...
static void devlink_traps_disable(struct devlink *devlink)
{
	...
	list_for_each_entry(trap_item, &devlink->trap_list, list) {
		trap_item->trap->state_set(devlink, trap_item->trap->id,
					   false);
		trap_item->state = DEVLINK_TRAP_STATE_DISABLE;
	}
}
			   ...
			   size_t traps_count)
{
	...
	/* Traps cannot report packets until all traps are registered. */
			      ...
			      size_t traps_count)
{
	...
	mutex_lock(&devlink->lock);
	/* Disable all the traps and wait for a grace period to make
	 * sure no packets are reported while the traps are being
	 * unregistered.
	 */
			    ...
			    const struct devlink_trap_group *group)
{
	...
	group_item->state = group->init_state;
 ...
 * @traps_count: Count of provided packet trap groups
 *
 * Return: Non-zero value on failure
 */
