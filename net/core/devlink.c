...
devlink_trap_group_item_lookup(struct devlink *devlink, const char *name)
{
	...
	list_for_each_entry(group_item, &devlink->trap_group_list, list) {
		if (strcmp(group_item->name, name) == 0)
			return group_item;
	}
...
__devlink_trap_group_action_set(struct devlink *devlink,
				...
				struct netlink_ext_ack *extack)
{
	const char *group_name = group_item->name;
	struct devlink_trap_item *trap_item;
	int err;
...
{
	if (!trap || !trap->name || !trap->group_name)
		return -EINVAL;
...
static struct devlink_trap_group_item *
devlink_trap_group_item_create(struct devlink *devlink, const char *group_name,
			       u16 group_id,
...
static struct devlink_trap_group_item *
devlink_trap_group_item_get(struct devlink *devlink, const char *group_name,
			    u16 group_id,
			    const struct devlink_trap_group_ops *group_ops)
{
	struct devlink_trap_group_item *group_item;
	...
	return devlink_trap_group_item_create(devlink, group_name, group_id,
					      group_ops);
}
			     ...
			     const struct devlink_trap_group_ops *group_ops)
{
	...
	group_item = devlink_trap_group_item_get(devlink, group_name, group_id,
						 group_ops);
			    ...
			    size_t skb_len)
{
	...
	return NLMSG_ALIGN(size) +
	       ...
	       nla_total_size(strlen(group_item->name) + 1) +
