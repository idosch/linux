...
static unsigned int icmp6_ext_max_len(u8 ext_objs)
{
	...
	if (ext_objs & BIT(ICMP_ERR_EXT_IIO_INC_IF))
...
static void icmp6_ext_iio_inc_if_append(struct net *net, struct sk_buff *skb,
					int iif)
{
	...
	objh->class_type = ICMP_EXT_CTYPE_IIO_ROLE_INC_IF;
...
{
	if (ext_objs & BIT(ICMP_ERR_EXT_IIO_INC_IF))
		icmp6_ext_iio_inc_if_append(net, skb, iif);
}
